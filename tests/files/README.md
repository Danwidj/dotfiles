...
- **`common.bats`**: Verifies cross-platform configurations: validates that `managed.zshrc`, `managed.zshenv`, `init.lua`, `tmux.conf`, `config.ghostty`, and `starship.toml` exist after apply, checks Zsh syntax and plugin order, and tests idempotency of `run_once_zshrc.sh` and `run_once_zshenv-shim.sh`.
